...
public:
  VectorUpdater(std::shared_ptr<const dolfinx::common::IndexMap> index_map)
  {
    ...
    MPI_Dist_graph_neighbors(
        comm, num_recv_neighbors, fwd_recv_neighbors.data(), nullptr,
  ...
  void update_fwd(la::Vector<double>& x)
  {
  ...
  void update_rev(la::Vector<double>& x)
  {
    ...
    // Step 2: begin scatter
    req.clear();
    req.resize(rev_recv_neighbors.size());
    for (std::size_t i = 0; i < req.size(); ++i)
    {
